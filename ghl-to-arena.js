...
export async function handler(event) {
  ...
  const { type, id, locationId, contactId } = payload;
  ...
  // Find existing lead by ghlId
  let targetMember = null;
  let existingIdx = -1;
  for (const member of arenaMembers) {
    const idx = pipelineData[member].leads.findIndex(l => l.ghlId === opp.id);
    if (idx !== -1) { targetMember = member; existingIdx = idx; break; }
  }

  // Match to assigned user
  if (!targetMember) {
    const assignedName = opp.user?.name || '';
    targetMember = arenaMembers.find(m => assignedName && m.toLowerCase().includes(assignedName.toLowerCase().split(' ')[0])) || '_ghl_unassigned';
  }
  ...
  if (existingIdx !== -1) {
    const kept = pipelineData[targetMember].leads[existingIdx];
    pipelineData[targetMember].leads[existingIdx] = { ...kept, ...newLead, followUpDate: kept.followUpDate || '', callDate: kept.callDate || '', activityLog: kept.activityLog || [] };
  } else {
    pipelineData[targetMember].leads.push(newLead);
  }

  await sbSet(PIPELINE_KEY, pipelineData);
  return { statusCode: 200, headers, body: JSON.stringify({ ok: true, synced: opp.id, member: targetMember }) };
}
